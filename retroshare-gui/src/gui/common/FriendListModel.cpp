...
QVariant RsFriendListModel::data(const QModelIndex &index, int role) const
{
    ...
    case StatusRole:         return statusRole(entry,index.column()) ;
...
{
    return statusRole(e,col) != RS_STATUS_OFFLINE;
}
...
QVariant RsFriendListModel::statusRole(const EntryIndex& e, int /*col*/) const
{
    switch(e.type)
    {
        default:
...
QVariant RsFriendListModel::fontRole(const EntryIndex& e, int col) const
{
    ...
    int status = statusRole(e,col).toInt();
