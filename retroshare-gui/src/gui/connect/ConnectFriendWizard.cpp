...
void ConnectFriendWizard::accept()
{
    ...
    if(rsEvents)
    {
        // one event for location, one for profile

        auto ev1 = std::make_shared<RsGossipDiscoveryEvent>();
        ev1->mGossipDiscoveryEventType = RsGossipDiscoveryEventType::PROFILES_CHANGED;
        rsEvents->postEvent(ev1);

        auto ev2 = std::make_shared<RsPeerStatusChangeEvent>();
        ev2->mSslId = peerDetails.id;
        ev2->mEventCode = RsPeerStatusEventCode::PEER_ADDED;
        rsEvents->postEvent(ev2);
    }
	QDialog::accept();
}
