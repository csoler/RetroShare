// COMPILE_LINE: g++ -c -g -o multiavatar.o multiavatar.cpp 
//
...
#define PARTV_ROBO       	0
#define PARTV_GIRL       	1
#define PARTV_BLONDE     	2
#define PARTV_GUY        	3
#define PARTV_COUNTRY    	4
#define PARTV_GEEKNOT    	5
#define PARTV_ASIAN      	6
#define PARTV_PUNK       	7
#define PARTV_AFROHAIR   	8
#define PARTV_NORMIE_FEMALE     9
#define PARTV_OLDER            10
#define PARTV_FIREHAIR         11
#define PARTV_BLOND            12
#define PARTV_ATEAM            13
#define PARTV_RASTA            14
#define PARTV_META             15
#define PARTV_COUNT            16
...
static std::string BinToHex(const char *arr, const uint32_t len)
{
	std::string out;
	...
	{
		char tmp[3];
		sprintf(tmp, "%02x", (int32_t) (((const uint8_t *) arr)[j]));
		out += std::string(tmp,2);
	}

	return out;
}
static std::string sha256sum(const unsigned char *data, uint32_t size)
{
	SHA256_CTX sha_ctx ;
	...
	SHA256_Final(&hash[0], &sha_ctx);

	return std::string((char*)hash,32);
}

static std::string hexdigest_sha256sum(const unsigned char *data, uint32_t size)
{
	return BinToHex(sha256sum(data,size).c_str(),32);
}

std::string multiavatar(const std::string& s,bool sansEnv) // def multiavatar (string, sansEnv, ver):
{
	// 
	// Color themes
	// 

	std::vector<std::vector<std::vector<std::vector<std::string> > > > themes = {
		...
		//# Blonde
		{
			...
			},
			{
				...
				{"#f70014"},
				{"#000", "magenta"},
				{"#FFCC00", "#FFCC00", "#FFCC00", "#ff0000", "yellow"}
			}
		},
		...
		//# Evilnormie
		{
			{
				{"#6FC30E"},
				{"#b4e1fa", "#5b5d6e", "#515262", "#a0d2f0", "#a0d2f0"},
				{"#fae3b9"},
				{"#fff", "#000"},
				{"#000"},
				{"#8eff45", "#8eff45", "none", "none"}
			},
			{
				{"#00a58c"},
				...
				{"#fff", "#000"},
				{"#000"},
				{"#FFC600", "none", "#FFC600", "none"}
			},
			{
				{"#ff501f"},
				{"#000", "#ff0000", "#ff0000", "#7d7d7d", "#7d7d7d"},
				{"#fff3dc"},
				{"#d2001b", "none"},
				{"#000"},
				{"#D2001B", "none", "none", "#D2001B"}
			}
		},
		...
		// # Country
		{
			{
				{"#fc0"},
				{"#901e0e", "#ffbe1e", "#ffbe1e", "#c55f54"},
				{"#f8d9ad"},
				{"#000", "none", "#000", "none"},
				{"#000"},
				{"#583D00", "#AF892E", "#462D00", "#a0a0a0"}
			},
			{
				{"#386465"},
				{"#fff", "#333", "#333", "#333"},
				...
				{"#000", "#000", "#000", "#000"},
				{"#000"},
				{"#27363C", "#5DCAD4", "#314652", "#333"}
			},
			{
				{"#DFFF00"},
				{"#304267", "#aab0b1", "#aab0b1", "#aab0b1"},
				{"#e6b876"},
				{"#50230a", "#50230a", "#50230a", "#50230a"},
				{"#000"},
				{"#333", "#afafaf", "#222", "#6d3a1d"}
			}
		},
		...
		// # Geeknot
		{
			{
				{"#a09300"},
				{"#c7d4e2", "#435363", "#435363", "#141720", "#141720", "#e7ecf2", "#e7ecf2"},
				{"#f5d4a6"},
				{"#000", "#cf9f76"},
				{"#000", "#000", "#000", "#000", "#000", "#000", "#fff", "#fff", "#fff", "#fff", "#000", "#000"},
				{"none", "#fdff00"}
			},
			{
				{"#b3003e"},
				{"#000", "#435363", "#435363", "#000", "none", "#e7ecf2", "#e7ecf2"},
				{"#f5d4a6"},
				{"#000", "#af9f94"},
				{"#9ff3ff;opacity:0.96", "#000", "#9ff3ff;opacity:0.96", "#000", "#2f508a", "#000", "#000", "#000", "none", "none", "none", "none"},
				{"#ff9a00", "#ff9a00"}
			},
			{
				{"#884f00"},
				{"#ff0000", "#fff", "#fff", "#141720", "#141720", "#e7ecf2", "#e7ecf2"},
				{"#c57b14"},
				{"#000", "#cf9f76"},
				{"none", "#000", "none", "#000", "#5a0000", "#000", "#000", "#000", "none", "none", "none", "none"},
				{"#efefef", "none"}
			}
		},
		...
		// # Asian
		{
			...
			},
			{
				...
				{"#000", "none"},
				{"#000"},
				{"#000","#000","#000", "none", "#ffb358",  "#000", "none", "none"}
			},
			{
				{"#ff184e"},
				{"#000", "none"},
				...
				{"#ff0000", "none"},
				{"#000"},
				{"none","none","none", "none", "none",  "#ffc107", "none", "none"}
			}
		},
		...
		{
			{
				...
				{"#ffce94"},
				{"#f73b6c", "#000"},
				{"#e91e63", "#000", "#e91e63", "#000", "#000", "#000"},
				{"#dd104f", "#dd104f", "#f73b6c", "#dd104f"}
			},
			{
				{"#181284"},
				{"#491f49", "#ff9809", "#491f49"},
				{"#f6ba97"},
				{"#ff9809", "#000"},
				{"#c4ffe4", "#000", "#c4ffe4", "#000", "#000", "#000"},
				{ "none", "none", "#d6f740", "#516303"}
			},
			{
				{"#bcf700"},
				{"#ff14e4", "#000", "#14fffd"},
				{"#7b401e"},
				{"#666", "#000"},
				{"#00b5b4", "#000", "#00b5b4", "#000", "#000", "#000"},
				{"#14fffd", "#14fffd", "#14fffd", "#0d3a62"}
			}
		},
		...
		// # Firehair
		{
			...
			{
				{"#00a08d"},
				{"#FFBA32", "#484848", "#4e4e4e", "#fff", "#fff"},
				{"#ab5f2c"},
				{"#191919", "#191919"},
				{"#000", "#ff23fa;opacity:0.39", "#000"},
				{"#ff90f4", "#ff90f4", "#ff90f4"}
			},
			{
				{"#22535d"},
				{"#000", "#ff2500", "#ff2500", "#fff", "#fff"},
				{"#a76c44"},
				{"#191919", "#191919"},
				{"#000", "none", "#000"},
				{"none", "#00efff", "none"}
		...
		// # Blond
		{
			{
				{"#2668DC"},
				{"#2385c6", "#b8d0e0", "#b8d0e0"},
				{"#ad8a60"},
				{"#000", "#4d4d4d"},
				{"#7fb5a2", "#d1eddf", "#301e19"},
				{"#fff510", "#fff510"}
			},
			{
				{"#643869"},
				{"#D67D1B", "#b8d0e0", "#b8d0e0"},
				{"#CC985A", "none0000"},
				{"#000", "#ececec"},
				{"#1f2644", "#9b97ce", "#301e19"},
				{"#00eaff", "none"}
			},
			{
				{"#F599FF"},
		...
		// # Ateam
		{
			{
				{"#d10084"},
				{"#efedee", "#00a1e0", "#00a1e0", "#efedee", "#ffce1c"},
				{"#b35f49"},
				{"#3a484a", "#000"},
				{"#000"},
				{"#000", "none", "#000", "none"}
			...
			{
				{"#E6C117"},
				{"#efedee", "#ec0033", "#ec0033", "#efedee", "#f2ff05"},
				{"#ffc016"},
				{"#4a3737", "#000"},
				{"#000"},
				{"#ffe900", "#ffe900", "none", "#ffe900"}
			},
			{
				{"#1d8c00"},
				{"#e000cb", "#fff", "#fff", "#e000cb", "#ffce1c"},
				{"#b96438"},
				{"#000", "#000"},
				{"#000"},
				{"#53ffff", "#53ffff", "none", "none"}
			}
		},
		...
		// # Rasta
		{
			{
				{"#fc0065"},
				{"#708913", "#fdea14", "#708913", "#fdea14", "#708913"},
				{"#DEA561"},
				{"#444", "#000"},
				{"#000"},
				{"#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f", "#32393f" }
			},
			{
				{"#81f72e"},
				{"#ff0000", "#ffc107", "#ff0000", "#ffc107", "#ff0000"},
				{"#ef9831"},
				{"#6b0000", "#000"},
				{"#000"},
				{"#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "#FFFAAD", "none", "none", "none", "none"}
			},
			{
				{"#00D872"},
				{"#590D00", "#FD1336", "#590D00", "#FD1336", "#590D00"},
				{"#c36c00"},
				{"#56442b", "#000"},
				{"#000"},
				{"#004E4C", "#004E4C", "#004E4C", "#004E4C", "#004E4C", "#004E4C", "#004E4C", "#004E4C", "#004E4C", "none", "none", "none", "none", "none", "none", "none", "none"}
			}
		},
		...
		// # Meta
		{
			...
			},
			{
				...
				{"#755227"},
				{"#fff", "#000"},
				{"black", "#1df7ff;opacity:0.64", "#fcff2c"},
				{"#fff539", "none", "#fff539", "none", "#fff539"}
			},
			{
				{"#DC75FF"},
				{"#000", "#FFBDEC"},
	...
	std::string hash( sha256sum((const unsigned char*)s.c_str(),s.length()).substr(0,6) ) ;
	...
	auto getFinal = [sP,themes](int part,int partV,int theme) -> std::string // def getFinal(part, partV, theme):
	{
		// # print(part, partV, theme)

		const auto& colors = themes[partV][theme][part];
		auto svgString = sP[partV][part];
		...
		while(std::regex_search(s,m,regex)) //result = re.findall(regex, svgString)
		{
			resultFinal += m.prefix();
			resultFinal += colors[i++] + ";" ;
	...
	std::string out = svgStart;

	if(!sansEnv)
		out += getFinal(PART_ENV,   ((unsigned char)hash[PART_ENV]  ) % PARTV_COUNT, ((unsigned char)hash[PART_ENV]  ) % THEME_COUNT);

	out += getFinal(PART_HEAD,  ((unsigned char)hash[PART_HEAD] ) % PARTV_COUNT, ((unsigned char)hash[PART_HEAD] ) % THEME_COUNT);
	out += getFinal(PART_CLO,   ((unsigned char)hash[PART_CLO]  ) % PARTV_COUNT, ((unsigned char)hash[PART_CLO]  ) % THEME_COUNT);
	out += getFinal(PART_TOP,   ((unsigned char)hash[PART_TOP]  ) % PARTV_COUNT, ((unsigned char)hash[PART_TOP]  ) % THEME_COUNT);
	out += getFinal(PART_EYES,  ((unsigned char)hash[PART_EYES] ) % PARTV_COUNT, ((unsigned char)hash[PART_EYES] ) % THEME_COUNT);
	out += getFinal(PART_MOUTH, ((unsigned char)hash[PART_MOUTH]) % PARTV_COUNT, ((unsigned char)hash[PART_MOUTH]) % THEME_COUNT);
